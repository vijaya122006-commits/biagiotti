"""
Generate train.ipynb — the interactive Jupyter notebook for the Cosmetic ML Pipeline.
Run this script once to create the notebook, then open train.ipynb in Jupyter.
"""
...
NOTEBOOK_PATH = Path(__file__).resolve().parent / "train.ipynb"
...
md("### 2.1 · Missing Values Overview")
code(
...
md("### 2.4 · Top Brands by Product Count")
code(
...
md("### 2.5 · Safety Score Distribution")
code(
...
md("### 2.6 · Sales Time Series Overview")
code(
    "date_col = None\n",
    "qty_col  = None\n",
...
md("### 7.1 · Build Weekly Time Series")
code(
...
md("### 7.3 · Random Forest Demand Forecasting")
code(
...
def build_notebook():
    ...
    for cell_type, source_lines in CELLS:
...
if __name__ == "__main__":
    ...
    print(f"  {len(nb['cells'])} cells  ({NOTEBOOK_PATH.stat().st_size / 1024:.1f} KB)")
